...
public class LecteurRequete implements StringRequete {
	...
	public String getViewAllAnswers(Connection conn, Utilisateur user, String query, String action, String...args) {
		try (PreparedStatement ps = conn.prepareStatement(query)) {			
			...
			try (ResultSet rs = ps.executeQuery()) {
				...
				System.out.println("");
				if (user.getCurrentNoQuestion() != -1) { // évite qu'on affiche la question si on veut juste l'ensemble des réponses 
					fullText += "RÉPONSES DE LA QUESTION N°"+Integer.toString(user.getCurrentNoQuestion())+" :\n";
					fullText += this.getViewQuestionContent(conn, StringRequete.stringTitreQuestion, "voir titre de la question", user);
				}
	...
	public String getViewQuestionContent(Connection conn, String query, String action, Utilisateur user) {
		try (PreparedStatement ps = conn.prepareStatement(query)) {
			ps.setString(1, Integer.toString(user.getCurrentNoQuestion()));
			String fullText = "", titreQuestion = "", contenuQuestion = "";
			try (ResultSet rs = ps.executeQuery()) {				
				while (rs.next()) {
					titreQuestion = rs.getString(6);
					contenuQuestion = rs.getString(7);
	...
	public String getHashedPassword(Connection conn, String pseudo) {
		String mdpDB = "";
		try (PreparedStatement ps = conn.prepareStatement(stringRecevoirMotDePasseHashe)) {
			...
			System.out.println("Erreur lors de : " + stringActionRecevoirMotDePasseHashe);
			System.out.println(e.getMessage());
			//e.printStackTrace();
		}
		return mdpDB;
